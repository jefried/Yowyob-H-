...
import 'package:flutter_whatsapp/Screens/InfoProfil.dart';
...
class IndividualPage extends StatefulWidget {
  IndividualPage({Key key, this.chatModel, this.numCurrent}): super(key: key);
...
class _IndividualPageState extends State<IndividualPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        Image.asset("assets/fontChat.jpg",
                ...
                PopupMenuButton<String>(
                  onSelected: (value) {
                    if(value=="Info Profil") {
                      Navigator.push(context, MaterialPageRoute(builder: (context) => InfoProfil()));
                    }
                  },
    ...
    child: Row(
      children: [
        CircleAvatar(
          radius: 20,
          child: widget.chatModel.urlPhoto == ""? SvgPicture.asset(
          ...
          backgroundImage: widget.chatModel.urlPhoto != ""? CachedNetworkImageProvider(
            widget.chatModel.urlPhoto,
          ) : null,
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if(messages[index].numSender == widget.numCurrent) {
                            return OwnMessageCard(message: messages[index].texte, time: DateHelper().getDate(messages[index].date), url: messages[index].urlMedia, sent: false, mediaType: messages[index].mediaType,);
                          } else {
                            return ReplyCard(message: messages[index].texte, time: DateHelper().getDate(messages[index].date), url: messages[index].urlMedia, sent: false, mediaType: messages[index].mediaType,);
                          }
                        },
                      ),
                    ),
                                      ...
                                      child: TextFormField(
                                        ...
                                        decoration: InputDecoration(
                                          ...
                                          suffixIcon: Row(
                                            ...
                                            children: [
                                              IconButton(icon: Icon(Icons.camera, color: Colors.indigo,),
                                                color: Colors.blueGrey,
                                                onPressed: () {
                                                  Navigator.push(context, MaterialPageRoute(builder: (builder) => CameraPage(from: "IndividualPage",)));
                                                },
                                              ),
                                              IconButton(icon: Icon(Icons.photo_library, color: Colors.indigo,),
                                                onPressed: () {
                                                  Navigator.push(context, MaterialPageRoute(builder: (builder) => Gallery(from: "IndividualPage",)));
                                                },
                                              ),
                                              Padding(
